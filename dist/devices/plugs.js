...
class Plug {
    constructor(platform, accessory, device) {
        ...
        rxjs_1.interval(this.platform.config.options.refreshRate * 1000)
            .pipe(operators_1.skipWhile(() => this.plugUpdateInProgress))
            .subscribe(() => {
            this.refreshStatus();
        });
    ...
    parseStatus() {
        ...
        this.platform.log.debug('Plug %s On: %s', this.accessory.displayName, this.On);
    }
    async refreshStatus() {
        try {
            this.platform.log.debug('Plug - Reading', `${settings_1.DeviceURL}/${this.device.deviceId}/status`);
            const deviceStatus = (await this.platform.axios.get(`${settings_1.DeviceURL}/${this.device.deviceId}/status`)).data;
            if (deviceStatus.message === 'success') {
                this.deviceStatus = deviceStatus;
                this.platform.log.warn('Plug %s refreshStatus -', this.accessory.displayName, JSON.stringify(this.deviceStatus));
                this.parseStatus();
                this.updateHomeKitCharacteristics();
            }
        }
        catch (e) {
            this.platform.log.error(`Plug - Failed to refresh status of ${this.device.deviceName}`, JSON.stringify(e.message), this.platform.log.debug('Plug %s -', this.accessory.displayName, JSON.stringify(e)));
            this.apiError(e);
        }
    }
    /**
